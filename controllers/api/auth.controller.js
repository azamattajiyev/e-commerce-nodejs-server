  ...
  async logout(req, res){
    try{
      ...
      jwt.verify(refresh_token, process.env.REFRESH_TOKEN_SECRET, (err, user) => {
        ...
        res.status(200).json(successRes(null,'user is logout'));
      })
      res.status(200).json(errorRes(`nabeli error`));
    }catch(e){
      console.log(e);
      res.status(200).json(errorRes(`Server error: ${e}`));
    }
  ...
  async token(req, res){
      try{
        ...
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET,async(err, user) => {
          ...
          return res.status(200).json(await getUserTokens(user.username));
        })
        res.status(200).json(errorRes(`nabeli error`));
      }catch(e){
        console.log(e);
